...
printf '\n\n'
echo "This script will take a directory of VCF files, preferably split by chromosome with each containing SNP information for at least one trio.  It will call variants using BCF Tools."
printf '\n\n'
...
processed=$(find $outdir -maxdepth 1 -name "*.raw_calls.vcf" -type f | sort)
...
screen -d -m -S "$fname".BCFtools bash -c "
$bcftoolsexp call -v --constrain trio -S $pedigree -m $i | $bcftoolsexp view --exclude-uncalled - | vcftools --max-missing 1.0 --vcf - --recode --out $outdir/$fname.raw;

sed -i '/Description="Genotype"/a ##FORMAT=<ID=GQ,Number=1,Type=Integer,Description="Genotype Quality"> ' $outdir/$fname.raw.recode.vcf;

java -jar $GATK \
 -T PhaseByTransmission \
 -R $mm10min \
 -V $outdir/$fname.raw.recode.vcf \
 -ped $pedigree \
 -mvf $fname.violations.txt \

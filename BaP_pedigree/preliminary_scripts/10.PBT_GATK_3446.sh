...
files=$(find $rootdir -maxdepth 1 -regex ".*.[bv]cf*" -type f ) # | grep -v old | grep -v analysis_files | grep $pattern | sort)
...
for i in $runlist
do
j=$(find $rootdir -maxdepth 1 -regex ".*$i\..*.[bv]cf*" -type f ) # | grep -v old | grep -v analysis_files)
FILEPATHS+=($j)
done

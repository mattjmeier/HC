...
$bcftoolsexp merge -O z -g $kid_gvcf $mom_gvcf $dad_gvcf | tee $child/$child.trio.merged.vcf.gz | $bcftoolsexp view | \
java -jar /utils/appz/snpEff/SnpSift.jar filter "isVariant( GEN[$child] ) & isRef( GEN[$mother] ) & isRef( GEN[$father] ) & isHom( GEN[$mother] ) & isHom( GEN[$father] )" > $child/$child.trio.violations.vcf
echo "Done."
...
while true
do
	java -jar $GATK3446    -T DepthOfCoverage -nt 12    -R $mm10min    -o /tmp/$child.sire.allele_depth    -I /tmp/$sire.list --printBaseCounts    -L /tmp/$child.bed
	pid1=$!
	java -jar $GATK3446    -T DepthOfCoverage -nt 12   -R $mm10min    -o /tmp/$child.dam.allele_depth    -I /tmp/$dam.list --printBaseCounts    -L /tmp/$child.bed
	pid2=$!
	java -jar $GATK3446    -T DepthOfCoverage -nt 12   -R $mm10min    -o /tmp/$child.kid.allele_depth    -I /tmp/$kid.list --printBaseCounts    -L /tmp/$child.bed
	pid3=$!
	wait $pid1
	wait $pid2	
	wait $pid3	
...
while true
do
	cat /tmp/$child.sire.allele_depth | grep "^chr" | awk '{ split($1,POS,":"); split($5,A,":"); split($6,C,":"); split($7,G,":"); split($8,T,":"); \
						print POS[1]":"POS[2]"\t"$4"\t"A[2]"\t"C[2]"\t"G[2]"\t"T[2] }' | sort -V > $child/$child.sire.basecounts
	pid1=$!
	cat /tmp/$child.dam.allele_depth | grep "^chr" | awk '{ split($1,POS,":"); split($5,A,":"); split($6,C,":"); split($7,G,":"); split($8,T,":"); \
						print POS[1]":"POS[2]"\t"$4"\t"A[2]"\t"C[2]"\t"G[2]"\t"T[2] }' | sort -V > $child/$child.dam.basecounts
	pid2=$!
	cat /tmp/$child.kid.allele_depth | grep "^chr" | awk '{ split($1,POS,":"); split($5,A,":"); split($6,C,":"); split($7,G,":"); split($8,T,":"); \
						print POS[1]":"POS[2]"\t"$4"\t"A[2]"\t"C[2]"\t"G[2]"\t"T[2] }' | sort -V > $child/$child.kid.basecounts
	pid3=$!
	wait $pid1
	wait $pid2	
	wait $pid3	
...
echo "Making annotations for allele frequencies..."
while true
do
	cat $child/$child.trio.violations.vcf | grep -v "^#" | awk ' { print $1":"$2"\t"$5 } ' | sort -V | join $child/$child.sire.basecounts - | \
		awk ' { if ($7 == "A") base=$3; else if ($7 == "C") base=$4; else if ($7 == "G") base=$5; else if ($7 == "T") base=$6; else base=" "; split($1,POS,":") ; \
		if (base == 0 || base == 1) print POS[1]"\t"POS[2]"\t"POS[2]"\t"base } ' > /tmp/$child.sire.annotations
	pid1=$!
	cat $child/$child.trio.violations.vcf | grep -v "^#" | awk ' { print $1":"$2"\t"$5 } ' | sort -V | join $child/$child.dam.basecounts - | \
		awk ' { if ($7 == "A") base=$3; else if ($7 == "C") base=$4; else if ($7 == "G") base=$5; else if ($7 == "T") base=$6; else base=" "; split($1,POS,":") ; \
		if (base == 0 || base == 1) print POS[1]"\t"POS[2]"\t"POS[2]"\t"base } ' > /tmp/$child.dam.annotations
	pid2=$!

## THIS NEEDS TO BE CONFIGURED TO RECORD ALLELE FRACTION OF MIN 25% ALT FOR KID

## NOTE!!! FIND OUT IF ANNOTATION FORMAT NEEDS TO BE ZERO BASED!!!

	cat $child/$child.trio.violations.vcf | grep -v "^#" | awk ' { print $1":"$2"\t"$5 } ' | sort -V | join $child/$child.kid.basecounts - | \
		awk ' { if ($7 == "A") base=$3; else if ($7 == "C") base=$4; else if ($7 == "G") base=$5; else if ($7 == "T") base=$6; else base=" "; split($1,POS,":") ; \
		af = base / $(NUMBER OF REFERENCE COUNTS); print POS[1]"\t"POS[2]"\t"POS[2]"\t"af ' > /tmp/$child.kid.annotations
#		if (base == 0 || base == 1) print POS[1]"\t"POS[2]"\t"POS[2]"\t"base } ' > /tmp/$child.kid.annotations
	pid3=$!
	wait $pid1
	wait $pid2	
	wait $pid3	
...
echo "Running bgzip for dam annotations file..."
bgzip /tmp/$child.kid.annotations
echo "Running tabix for dam annotations file..."
tabix -fs 1 -b 2 -e 3 /tmp/$child.kid.annotations.gz

echo "Applying sire annotations file to $kid_gvcf..."
cat $child/$child.trio.violations.vcf | \
	vcf-annotate -a /tmp/$child.sire.annotations.gz \
	...
	-c CHROM,FROM,TO,INFO/SIRE_AF_FILTER | \
	vcf-annotate -a /tmp/$child.dam.annotations.gz \
	...
	vcf-annotate -a /tmp/$child.kid.annotations.gz \
	-d key=INFO,ID=KID_AF_FILTER,Number=A,Type=Integer,Description='This means the Child has been found to have a minimum X percentage of the de novo alternate allele.' \
	-c CHROM,FROM,TO,INFO/SIRE_AF_FILTER | \
...
cat $child/$child.Sire_and_Dam_AF_flagged.vcf | grep -v "\./\." | java -jar /utils/appz/snpEff/SnpSift.jar filter " (DAM_AF_FILTER == 0) & (SIRE_AF_FILTER == 0) "  \
 | tee $child/$child.MVs.NO_MISSING.vcf | java -jar /utils/appz/snpEff/SnpSift.jar filter " ( QUAL > 10 ) & ( MIN_DP > 10 ) " | tee $child/$child.MVs.QUAL_10.MIN_DP_10.NO_MISSING.vcf | grep -v "^#" | \
 awk '{print $1","$2}' > $child/$child.MVs.QUAL_10.MIN_DP_10.NO_MISSING.csv

...
# Get base info
java -jar $GATK3446    -T DepthOfCoverage    -R $mm10min    -o /tmp/$vcf.allele_depth    -I /tmp/$sire.list --printBaseCounts    -L /tmp/$vcf.bed
...
cat /tmp/$vcf.allele_depth | grep "^chr" | awk '{ split($1,POS,":"); split($5,A,":"); split($6,C,":"); split($7,G,":"); split($8,T,":"); print POS[1]":"POS[2]"\t"$4"\t"A[2]"\t"C[2]"\t"G[2]"\t"T[2] }' | sort -V > $vcf.basecounts

# Parse the allele depth (basecounts) output from GATK; look at 
cat $vcf | grep -v "^#" | awk ' { print $1":"$2"\t"$5 } ' | sort -V | join $vcf.basecounts - | awk ' { if ($7 == "A") base=$3; else if ($7 == "C") base=$4; else if ($7 == "G") base=$5; else if ($7 == "T") base=$6; else base=" "; split($1,POS,":") ; if (base == 0 || base == 1) print POS[1]"\t"POS[2]"\t"POS[2]"\t"base } ' > /tmp/$vcf.annotations
...
bgzip /tmp/$vcf.annotations
tabix -s 1 -b 2 -e 3 /tmp/$vcf.annotations.gz

cat $vcf | vcf-annotate -a /tmp/$vcf.annotations.gz \

...
#read -e rootdir
#rootdir=/s0/ngs/DNA/ref_gen/mutamouse/illumina/GenomeQuebecFASTQs/
rootdir=/s0/ngs/DNA/ref_gen/mutamouse/illumina/GenomeQuebecNextSeq
echo $rootdir

echo "What directory would you like to put the output files in? Please provide the full path."
#read -e outdir
outdir=~/DNA/data/mutamouse/lacz/lambdagt10_HC/alignments/mutamouse14_reads/HC_animals/
echo $outdir
...
/utils/appz/bwa/bwa.kit/bwa mem -M -t $numthreads -R '@RG\tID:"$ID"\tSM:"$SM"\tPL:"$PL"\tLB:"$LB"\tPU:"$PU"' ~/DNA/data/mutamouse/lacz/lambdagt10_HC/health_canada_mutamouse_manual_assembly-2006.fa $i $i2 |  /utils/appz/samtools/samtools-1.3/samtools view -F 4 -bh > $outdir/$fname.bam
